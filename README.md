# rolecards6-docs

PRIVACY POLICY

Last updated: September 4, 2026

RoleCards ("the App") is developed by Dmytro Ostapchenko.

The App does not require an account and collects no personal data. We do not collect your name, email, phone number, location, contacts, photos, or any device or advertising identifier such as IDFA or IDFV. We do not track you across other apps or websites and we share nothing with advertisers or data brokers.

The App records anonymous usage events — for example, that a screen was opened or a game was started — together with basic technical context such as the App version and operating system version. These events are not linked to you or your device and are analysed only in aggregate, to help us decide what to improve. They are processed on our behalf by [ANALYTICS PROVIDER].

Because the data contains no identifiers, it cannot be traced to an individual, and we are therefore unable to retrieve or delete records on request. If you have questions about this policy, or about your rights under the GDPR, write to [YOUR EMAIL].

Apple separately collects information about downloads, purchases and crashes under its own privacy policy; we receive it only in aggregate form.

We may update this policy. The date above shows the last revision.

TERMS OF USE

Last updated: September 4, 2026

By using RoleCards ("the App") you agree to these terms. If you do not agree, do not use the App.

We grant you a personal, non-exclusive, revocable licence to use the App on devices you own, in line with the App Store Terms of Service. You may not copy, modify, reverse engineer, resell or sublicense the App, or use it unlawfully. The App and its content remain the property of [YOUR NAME].

The App is provided "as is", without warranty of any kind. To the fullest extent permitted by law, we are not liable for any indirect or consequential damages arising from your use of it. We may modify or discontinue the App at any time.

These terms are governed by the laws of Ukraine. Apple is not a party to these terms and has no obligation to provide support for the App.
